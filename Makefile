...
RAYLIB_VERSION        ?= 5.0
RAYLIB_URL			  ?= https://codeload.github.com/raysan5/raylib/tar.gz/refs/tags/5.0
RAYLIB_PATH           ?= ./external/raylib-5.0

# Define raylib source code path
RAYLIB_SRC_PATH       ?= ./external/raylib-5.0/src
...
RAYLIB_LIB 			  ?= ./external/raylib-5.0/src/libraylib.a
RAYLIB_HEADER 		  ?= ./external/raylib-5.0/src/raylib.h
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ...
    ifeq ($(PLATFORM_OS),BSD)
        LDFLAGS += -Lsrc -L$(RAYLIB_SRC_PATH)
    endif
endif
ifeq ($(PLATFORM),PLATFORM_DESKTOP_SDL)
    ifeq ($(PLATFORM_OS),WINDOWS)
...
all: fetch_and_build_raylib game_of_life

fetch_and_build_raylib:
	@if [ ! -f $(RAYLIB_LIB) ] || [ ! -f $(RAYLIB_HEADER) ]; then \
	    echo "Raylib not found, fetching and building..."; \
	    mkdir -p external; \
	    wget -O "./external/raylib-5.0.tar.gz" "https://codeload.github.com/raysan5/raylib/tar.gz/refs/tags/5.0"; \
	    tar xf "./external/raylib-5.0.tar.gz" -C "./external/"; \
	    rm "./external/raylib-5.0.tar.gz"; \
	    cd "./external/raylib-5.0/src" && make; \
	else \
	    echo "Raylib is already up-to-date."; \
	fi

# Define source code object files required
#------------------------------------------------------------------------------------------------
game_of_life: src/main.c
ifeq ($(PLATFORM),PLATFORM_ANDROID)
